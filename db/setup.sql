...
DO $$
DECLARE
    vorboss_latitude NUMERIC(9,6) := 51.520168;
    vorboss_longitude NUMERIC(9,6) := -0.081597;
BEGIN
    ...
    FROM (
        SELECT
            LEFT('VORB-' || chr(trunc(random() * 26)::int + 65) || chr(trunc(random() * 26)::int + 65) || '' || trunc(random() * 10000), 10) AS id,
            vorboss_latitude + (random() * 0.1 * (CASE WHEN random() < 0.5 THEN -1 ELSE 1 END)) AS latitude,
            vorboss_longitude + (random() * 0.1 * (CASE WHEN random() < 0.5 THEN -1 ELSE 1 END)) AS longitude,
    ...
    FROM (
        SELECT
            LEFT('VORB-' || chr(trunc(random() * 26)::int + 65) || chr(trunc(random() * 26)::int + 65) || '' || trunc(random() * 10000), 10) AS id,
            vorboss_latitude + (random() * 5)  * (CASE WHEN random() < 0.5 THEN -1 ELSE 1 END)AS latitude,
            vorboss_longitude + (random() * 5)  * (CASE WHEN random() < 0.5 THEN -1 ELSE 1 END)AS longitude,
            FLOOR(random() * 12) * 10 AS total_capacity,
            FLOOR(random() * 8) * 10 AS used_capacity
